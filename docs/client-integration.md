...
- `tools/list` returns the *enabled* tools in a fixed order (`sanitize`, `restore`, `inspect`, `forget`) with `ttlMs: 300000` and `cacheScope: private` (session-scoped results must not sit in shared caches).
...
- **Model visibility (threat model)**: MCP tool results are returned to the *calling model*, so a model-driven `context.restore` hands raw originals into the very context this boundary protects — a prompt-injected model can call it on the sanitized text it holds, or trick a harness into restoring fabricated placeholders. `mcp-stdio` therefore exposes only `context.sanitize` and `context.inspect` by default; enable more with `--tools restore,forget` or `--tools all` when a client explicitly needs it. Restore harness-side with `do-context-shield restore --session <id>` (same vault flags as the server) so originals never enter the model loop. `restore` resolves only the exact current token held by the same vault/session; legacy built-in tokens are rejected, so only restore text you sanitized with the current format.
- Tool arguments are enforced, not just declared: a non-object `arguments`, or a missing/non-string `text` for the tools that declare it, is an error, never a silent empty-input call. `context.sanitize` without a `session` argument is the single documented fallback; the CLI requires `--session` on `sanitize`, `restore`, and `forget`.
...
- Plugin selection: `--detector regex|gliner2|hybrid|process`, `--policy default|process`, `--transformer pseudonymize|process`, `--vault memory|json|process`, plus `--detector-command`, `--policy-command`, `--transformer-command`, `--vault-command`, and `--process-timeout-ms` for process plugins (`docs/process-plugin.md`). `--detector gliner2` and `--detector hybrid` also need `--model-dir <dir>` and a binary built with `--features gliner2` (`docs/plugins.md` covers the supported ONNX layouts, the hybrid pairing, and the recommended models); `--vault json` needs `--vault-file <path>` and cannot be combined with `--vault process`.
