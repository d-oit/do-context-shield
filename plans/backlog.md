...
- **Remaining work**: register the server in `~/.codex/config.toml` or a project `.codex/config.toml`, run a non-interactive Codex turn that calls `context.inspect` + `context.sanitize` (session `codex-probe-1`), confirm the reply carries placeholders only, and record the exact client flags that were needed.
...
- **Remaining work**: with working credentials, run a `claude -p` turn that calls
  `context.inspect` + `context.sanitize` (session `cc-probe-1`) and confirm the reply carries
